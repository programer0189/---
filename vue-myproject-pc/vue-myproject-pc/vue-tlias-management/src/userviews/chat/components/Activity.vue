<template>
  <div class="activity-container">
    ...
    <div class="page-header">
      <h2>活动组织</h2>
      <p class="subtitle">组织校园活动，丰富校园生活</p>
    </div>
    ...
    <!-- 发帖对话框 -->
    <el-dialog v-model="postDialogVisible" title="发布活动信息" width="600px">
      <el-form :model="postForm" :rules="postRules" ref="postFormRef" label-width="80px">
        ...
        <el-form-item label="内容" prop="content">
          <el-input 
            ...
            type="textarea" 
            :rows="8"
            placeholder="请详细描述活动信息、时间、地点、参与方式等..."
            maxlength="1000"
            show-word-limit
          />
        </el-form-item>
          ...
          <div class="post-list">
            <div v-for="post in userStats.recentPosts" :key="post.id" class="post-item-mini">
              <div class="post-info">
                ...
                <!-- 显示拒绝原因 -->
                <div v-if="post.judgeStatus === 2 && post.rejectReason" class="reject-reason">
    ...
    if (valid) {
      ...
      try {
        ...
        if (response.code === 1) {
          ...
          // 立即刷新用户统计对话框（如果当前打开）
          if (userStatsDialogVisible.value) {
            await loadUserStats()
          }
        } else {
...
const loadPosts = async () => {
  const response = await postAPI.getPostsByType(4) // 活动组织类型
...
.user-info-section {
  ...
  background: linear-gradient(135deg, #f7af44 0%, #e3b21e 100%);
  padding: 20px;
  border-radius: 15px;
  margin-bottom: 30px;
  color: white;
