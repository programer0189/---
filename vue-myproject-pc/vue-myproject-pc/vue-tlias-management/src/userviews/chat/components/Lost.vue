<template>
  <div class="lost-container">
    ...
    <div class="page-header">
      <h2>失物招领</h2>
      <p class="subtitle">帮助寻找失物，传递校园温暖</p>
    </div>
    ...
    <!-- 发帖对话框 -->
    <el-dialog v-model="postDialogVisible" title="发布失物招领" width="600px">
      <el-form :model="postForm" :rules="postRules" ref="postFormRef" label-width="80px">
        ...
        <el-form-item label="内容" prop="content">
          <el-input 
            ...
            type="textarea" 
            :rows="8"
            placeholder="请详细描述物品特征、丢失/拾获地点、联系方式等信息..."
            maxlength="1000"
            show-word-limit
          />
        </el-form-item>
...
const loadPosts = async () => {
  const response = await postAPI.getPostsByType(4) // 失物招领类型
...
.user-info-section {
  ...
  background: linear-gradient(135deg, #fdfafd 0%, #1b7095 100%);
  padding: 20px;
  border-radius: 15px;
  margin-bottom: 30px;
  color: white;
...
.post-btn {
  ...
  font-size: 18px;
  border-radius: 25px;
  background: linear-gradient(45deg, #342836, #e7e1e3);
  border: none;
