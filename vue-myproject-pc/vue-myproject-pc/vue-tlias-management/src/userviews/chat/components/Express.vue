<template>
  <div class="express-container">
    ...
    <div class="page-header">
      <h2>代取快递</h2>
      <p class="subtitle">互助取快递，让校园生活更便利</p>
    </div>
    ...
    <!-- 发帖对话框 -->
    <el-dialog v-model="postDialogVisible" title="发布代取快递" width="600px">
...
const loadPosts = async () => {
  const response = await postAPI.getPostsByType(2) // 代取快递类型
...
.user-info-section {
  ...
  background: linear-gradient(#d3d1d1,#1e70d5 100%);
  padding: 20px;
  border-radius: 15px;
  margin-bottom: 30px;
  color: white;
...
.post-btn {
  ...
  font-size: 18px;
  border-radius: 25px;
  background: linear-gradient(45deg, #fc0000, #de6161);
  border: none;
