<template>
  <div class="confession-container">
    ...
    <div class="page-header">
      <h2>活动组织</h2>
      <p class="subtitle">组织活动，让生活更加精彩</p>
    </div>
...
const loadPosts = async () => {
  ...
  const response = await postAPI.getPostsByType(4) // 活动类型
  console.log('加载的帖子数据:', response.data) // 添加调试信息
  postsList.value = response.data.map(post => {
    console.log('处理帖子:', post.id, 'createTime:', post.createTime) // 添加调试信息
    return {
      ...post,
      isLiked: false,
      showComments: false,
