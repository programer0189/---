<template>
    <div class="confession-container">
      <!-- 页面头部 -->
      <div class="back-btn" @click="goToUserInfo">
      ...
      <div class="page-header">
        ...
        <p class="subtitle">查看您发布的所有帖子内容</p>
...
const handleImageError = (event) => {
  ...
  const loadPosts = async () => {
    ...
    const response = await postAPI.getUserPosts(currentUser.value.id) // 获取当前用户的所有帖子
  ...
  .confession-container {
   ...
   .back-btn {
     ...
     gap: 8px;
     padding: 12px 20px;
     background: linear-gradient(135deg, #eee8e8 0%, #e4e1e1 100%);
     color: rgb(56, 55, 55);
     border: none;
     border-radius: 25px;
     font-size: 16px;
